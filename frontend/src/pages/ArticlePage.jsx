  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        {catLabel && (
          <span className="text-[9px] font-bold uppercase tracking-wider text-[#2563EB] block mb-0.5">{catLabel}</span>
        )}
...
export default function ArticlePage() {
  ...
  const shareUrl = `https://www.siliconfund.in/news/${id}`;

  useEffect(() => {
    if (title) document.title = `${title} - Silicon Fund`;
    return () => { document.title = "Silicon Fund"; };
  }, [title]);
  ...
  if (loading) return (
    <div className={`min-h-screen flex items-center justify-center ${darkMode ? "bg-[#0A0F1E]" : "bg-white"}`}>
      <div className="animate-spin w-8 h-8 border-2 border-[#2563EB] border-t-transparent rounded-full" />
    </div>
  );

  if (!article) return (
    <div className={`min-h-screen flex flex-col items-center justify-center gap-4 ${darkMode ? "bg-[#0A0F1E] text-white" : "bg-white"}`}>
      <p>Article not found</p>
      <button onClick={() => navigate("/")} className="text-[#2563EB]">Go Home</button>
    </div>
  );
  ...
  return (
    <div data-testid="article-page" className={`min-h-screen pb-10 ${darkMode ? "bg-[#0A0F1E]" : "bg-white"}`}>
      {/* Reading progress bar */}
      <div className="fixed top-0 left-0 z-[70] h-[3px] bg-gradient-to-r from-[#2563EB] to-[#06B6D4] transition-all duration-100" style={{ width: `${progress}%` }} />
      {/* Sticky top bar */}
      <div className={`sticky top-0 z-10 px-4 py-3 flex items-center gap-3 border-b ${darkMode ? "bg-[#0A0F1E]/95 border-slate-800" : "bg-white/95 border-slate-100"} backdrop-blur`}>
        ...
        <span className={`text-sm font-semibold flex-1 truncate ${darkMode ? "text-white" : "text-slate-800"}`}>Silicon Fund</span>
      ...
      <div className="max-w-screen-xl mx-auto px-4 py-6 lg:grid lg:grid-cols-[260px_1fr_260px] lg:gap-8">
        ...
        <aside className="hidden lg:block">
          <div className={`sticky top-20 rounded-xl p-4 ${darkMode ? "bg-[#111827] border border-slate-800" : "bg-white border border-slate-200"}`}>
            <h3 className={`text-[10px] font-black uppercase tracking-[0.18em] pb-2 mb-3 border-b-2 border-[#2563EB] ${darkMode ? "text-slate-300" : "text-slate-700"}`}>
              Related Stories
            </h3>
        ...
        <article className="min-w-0">
          ...
          <div className="flex items-center gap-2 mb-3">
            <span className="px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-[#2563EB] text-white rounded">
              {category}
            </span>
            {publishedTime && (
            ...
            <span className={`text-[11px] ${darkMode ? "text-slate-500" : "text-slate-400"}`}>· {readTime} min read</span>
          </div>
          ...
          {article.image && (
            <div className="relative mb-6 rounded-xl overflow-hidden">
              ...
              {category && (
                <span className="absolute bottom-4 left-4 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider bg-[#2563EB] text-white rounded">
                  {category}
                </span>
              )}
            </div>
          ...
          <div className={`prose prose-lg max-w-none ${darkMode ? "prose-invert" : ""}`}>
            {summary.split('\n').filter(p => p.trim()).map((para, i) => (
              <p key={i} className={`text-[17px] leading-[1.85] mb-5 ${darkMode ? "text-slate-300" : "text-slate-700"}`}>
                {para}
              </p>
            ))}
          </div>
          ...
          <div className={`mt-12 pt-8 border-t ${darkMode ? "border-slate-800" : "border-slate-200"}`}>
            <div className="flex items-center gap-3 mb-1">
              <div className="w-1 h-5 bg-[#2563EB] rounded-full" />
              <h3 className={`text-[13px] font-black uppercase tracking-[0.18em] ${darkMode ? "text-slate-300" : "text-slate-600"}`}>
                Continue Reading
              </h3>
            </div>
            <div className="flex gap-3 overflow-x-auto hide-scrollbar pt-4 pb-2">
              {relatedArticles.map(a => (
                <div key={a.id || a._id} onClick={() => openArticle(a)}
                  className={`flex-shrink-0 w-[220px] rounded-xl overflow-hidden border cursor-pointer hover:border-[#2563EB] transition-colors ${darkMode ? "bg-[#111827] border-slate-800" : "bg-white border-slate-200"}`}>
                  <img src={a.image || ""} alt={a.title} className="w-full h-28 object-cover" onError={e => e.target.style.display='none'} />
                  <div className="p-3">
                    <p className={`text-[12px] font-bold leading-snug line-clamp-3 ${darkMode ? "text-slate-200" : "text-slate-800"}`}>{a.title}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Newsletter CTA */}
          <div className="mt-8 rounded-2xl overflow-hidden bg-gradient-to-r from-[#2563EB] to-[#6366F1] p-6 text-white">
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-blue-200 mb-1">Silicon Fund Daily</p>
            <h3 className="text-[20px] font-bold mb-2">Intelligence that moves capital.</h3>
            <p className="text-[13px] text-blue-100 mb-4">5-minute brief on deals, IPOs and deep tech — every weekday at 7am IST.</p>
            <div className="flex gap-2">
              <input type="email" placeholder="your@email.com" className="flex-1 rounded-full px-4 py-2 text-[13px] text-slate-900 outline-none bg-white/90" />
              <button className="bg-[#06B6D4] px-4 py-2 rounded-full text-[13px] font-bold whitespace-nowrap hover:bg-[#0891B2] transition-colors">Subscribe →</button>
            </div>
          </div>
