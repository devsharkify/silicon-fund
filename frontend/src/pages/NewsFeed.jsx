  ...
  return (
    ...
    >
      <div className="relative aspect-[16/10] overflow-hidden rounded-md">
        ...
        />
        {article.is_pinned && (
          <span className="absolute top-3 left-3 bg-[#06B6D4] text-white text-[10px] font-bold px-2.5 py-1 rounded uppercase tracking-wider">
            Breaking
          </span>
        )}
      </div>
      ...
      <div className="pt-4">
        ...
        <div className="flex items-center gap-2 text-[12px] text-ink-muted">
          <span className="font-semibold">By Silicon Fund</span>
...
function MarketsTile({ articles }) {
  ...
  return (
    <div className="lg:col-span-1 lg:row-span-1 bg-gradient-to-br from-[#2563EB] to-[#4F46E5] text-white rounded-md overflow-hidden flex flex-col">
      {/* Header */}
      <div className="px-4 pt-4 pb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.18em]">Deal Flow</span>
        </div>
        <button
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-1.5">
                    <span className="text-[10px] opacity-50 font-medium">{article.source || "Silicon Fund"}</span>
                    {article.category && (
                      <>
...
export default function NewsFeed() {
  ...
  // Fetch trending articles once on mount
  useEffect(() => {
    axios.get(`${API}/news/trending?limit=6`)
      .then(res => setTrendingArticles(res.data.articles || res.data || []))
      .catch(() => {});
  }, []);
  ...
  return (
    <div data-testid="news-feed-page" className={`min-h-screen pb-20 ${darkMode ? "bg-[#0A0F1E]" : "bg-[#F5F7FF]"}`}>
      {/* ── Section A: Combined Search + Filter pill bar ── */}
      <div className={`flex items-center gap-2 py-3 px-4 sticky top-[100px] z-30 border-b shadow-sm ${darkMode ? "bg-[#0D1526] border-[#1E293B]" : "bg-white border-[#E2E8F0]"}`}>
        <div className="relative flex-1 flex items-center">
          ...
          <input
            ...
            placeholder="Search deals, startups, founders..."
            className={`w-full pl-10 pr-28 py-2 text-sm rounded-full border outline-none transition-all focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/20 ${
              darkMode
                ? "border-[#1E293B] bg-[#111827] text-slate-200 placeholder:text-slate-500"
                : "border-[#E2E8F0] bg-[#F5F7FF] text-ink placeholder:text-ink-muted"
            }`}
          />
          {isSearching && !loading && (
            <span className="absolute right-10 text-[11px] text-[#2563EB] font-medium pointer-events-none">
          ...
          {searchInput && (
            <button
              ...
              onClick={clearSearch}
              className="absolute right-3 p-0.5 rounded-full text-ink-muted hover:text-ink hover:bg-[#E2E8F0]/60"
            >
              <X size={14} />
            </button>
          )}
        ...
        <select
          ...
          className={`text-[12px] font-semibold rounded-full border px-3 py-2 outline-none cursor-pointer transition ${
            darkMode
              ? "border-[#1E293B] bg-[#111827] text-slate-300 hover:border-[#2563EB]"
              : "border-[#2563EB]/40 bg-white text-ink hover:border-[#2563EB]"
          }`}
        >
      ...
      {!loading && articles.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24">
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-4 bg-[#2563EB]/10">
            <Newspaper size={28} className="text-[#2563EB]" />
          </div>
      ...
      {!loading && articles.length > 0 && (
        <>
          ...
          <div className="max-w-screen-xl mx-auto px-4">
            ...
            {trendingArticles.length > 0 && (
              <div className="mb-6">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className={`text-[10px] font-black uppercase tracking-[0.18em] ${darkMode ? "text-[#06B6D4]" : "text-[#06B6D4]"}`}>Trending Now</span>
                </div>
                <div className="flex gap-3 overflow-x-auto pb-2 hide-scrollbar">
                  {trendingArticles.map((article, i) => (
                      <button
                        key={article.id || article._id}
                        onClick={() => navigate(`/news/${article.id || article._id}`)}
                        className={`flex-shrink-0 w-52 text-left rounded-xl border overflow-hidden group transition-all hover:border-[#2563EB] hover:shadow-md ${
                          darkMode ? "bg-[#111827] border-[#1E293B]" : "bg-white border-[#E2E8F0]"
                        }`}
                      >
                        {article.image && (
                          <div className="h-28 overflow-hidden">
                            <img src={article.image} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" onError={e => e.target.style.display='none'} />
                          </div>
                        )}
                        <div className="p-3">
                          <span className="text-[9px] font-bold uppercase tracking-wider text-[#2563EB] block mb-1">#{i + 1} Trending</span>
                          <p className={`text-[12px] font-semibold line-clamp-2 leading-snug ${darkMode ? "text-slate-200" : "text-[#0F172A]"}`}>{article.title}</p>
                        </div>
                      </button>
                  ))}
                </div>
            ...
            <div className="border-t-2 border-[#2563EB] mb-1" />
            <div className="flex justify-between items-baseline mb-5 pt-3">
              <h2 className={`font-display text-[20px] font-bold ${darkMode ? "text-slate-100" : "text-ink"}`}>
                {sectionTitle}
              </h2>
            </div>
            ...
            {hasMore && (
              <div className="flex justify-center mt-10 mb-6">
                <Button
                  ...
                  className="bg-[#2563EB] hover:bg-[#1D4ED8] text-white rounded-full px-8 py-3 text-[13px] font-bold uppercase tracking-wider transition-all active:scale-95"
                >
                  {loadingMore ? (
                    <>
