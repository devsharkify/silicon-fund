...
const BACKEND_URL = process.env.REACT_APP_BACKEND_URL || "https://mint-street-production.up.railway.app"; // data backend
...
function AppContent() {
  const darkMode = false; // Light mode only
  ...
  const toggleDarkMode = useCallback(() => {}, []); // no-op — light mode only
  ...
  if (isAdminPage && !isAdmin) {
    return (
      <AppContext.Provider value={contextValue}>
        <div className={`min-h-screen flex items-center justify-center ${darkMode ? "dark bg-[#0A0F1E]" : "bg-[#F5F7FF]"}`}>
          <div className="text-center p-8">
            ...
            <button onClick={() => navigate("/")} className="text-[#2563EB] font-medium">Go to Home</button>
          </div>
  ...
  return (
    <AppContext.Provider value={contextValue}>
      <div className={`min-h-screen ${darkMode ? "dark bg-[#0A0F1E]" : "bg-[#F5F7FF]"}`}>
        <ScrollToTop />
