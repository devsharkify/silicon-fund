...
export const ArticleModal = () => {
  ...
  const handleShare = () => {
    // Share the branded siliconfund.in URL so the unfurl card shows the Silicon Fund domain.
    const shareUrl = `https://www.siliconfund.in/news/${article.id}`;
    ...
    >
      ...
      {hasPrev && (
        <button data-testid="swipe-prev-btn" onClick={() => { setDirection(-1); goPrevArticle(); }}
          className="absolute left-1 md:left-4 top-1/2 -translate-y-1/2 z-[60] text-black hover:text-[#2563EB] transition-colors p-1">
          <ChevronLeft size={32} strokeWidth={2.5} />
        </button>
      )}
      {hasNext && (
        <button data-testid="swipe-next-btn" onClick={() => { setDirection(1); goNextArticle(); }}
          className="absolute right-1 md:right-4 top-1/2 -translate-y-1/2 z-[60] text-black hover:text-[#2563EB] transition-colors p-1">
          ...
          <div className="flex gap-1">
            {articlesList.slice(Math.max(0, articleIndex - 3), Math.min(articlesList.length, articleIndex + 4)).map((a, i) => {
              ...
              return <div key={a.id} className="rounded-full transition-all" style={{
                ...
                height: "5px",
                background: realIdx === articleIndex ? "#2563EB" : "rgba(255,255,255,0.3)",
              }} />;
            })}
          </div>
        </div>
      ...
      <AnimatePresence mode="popLayout" custom={direction}>
        ...
        >
          ...
          <div className={`sticky top-0 z-20 border-b px-4 py-3 flex items-center justify-between ${
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button data-testid="modal-save-btn" onClick={() => saveArticle(article)}
                className={`h-9 w-9 rounded-full flex items-center justify-center transition-colors ${isSaved ? "bg-[#2563EB] text-white" : darkMode ? "text-slate-300 hover:bg-slate-700" : "text-slate-500 hover:bg-slate-100"}`}>
          ...
          <div className="relative" onClick={handleDoubleTap} data-testid="article-doubletap-area">
            ...
            {/* Silicon Fund Logo Watermark - bottom-right */}
            <div className="absolute bottom-2 right-2 pointer-events-none select-none">
              <div className="flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-black/45 backdrop-blur-sm">
                <img src="/logo.svg" alt="Silicon Fund" className="h-4 w-auto" />
                <span className="text-[10px] font-bold text-white uppercase tracking-wider">Silicon Fund</span>
              </div>
            </div>
          ...
          <div className="p-5">
            <div className="flex items-center gap-3 mb-3">
              <span className="px-2 py-1 rounded text-xs font-semibold text-white bg-[#2563EB]">
                {categoryLabel}
              </span>
              {article.is_pinned && (
