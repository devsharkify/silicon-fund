...
import os
import asyncio
from database import client, db, CATEGORIES
...
# ===================== NEW SILICON FUND ENDPOINTS =====================

# Feed projection reused across new endpoints
_SF_PROJECTION = {
...
async def get_trending_articles(limit: int = 10):
    """Returns most recent articles as a proxy for trending (sorted by view_count if available, else by created_at)."""
    limit = min(limit, 20)
    try:
        articles = await db.news.find(
            {"is_active": True},
            _SF_PROJECTION,
        ).sort([("view_count", -1), ("created_at", -1)]).limit(limit).to_list(limit)
        return articles
    except Exception:
...
async def get_spotlight_articles():
    """Returns 1 featured (newest) article per major category."""
    spotlight_categories = ["funding", "ipo", "startups", "deeptech", "fintech", "policy", "vc"]
    result = {}
    for cat in spotlight_categories:
        articles = await db.news.find(
            {"is_active": True, "category": cat},
            _SF_PROJECTION,
        ).sort([("published_at", -1)]).limit(1).to_list(1)
        if articles:
            result[cat] = articles[0]
    return result


@app.get("/api/stats/deals")
async def get_deal_stats():
    """Returns deal/article statistics including total, today, and per-category counts."""
    total_articles = await db.news.count_documents({"is_active": True})

    now = datetime.now(timezone.utc)
    since_24h = (now - timedelta(hours=24)).isoformat()
    articles_today = await db.news.count_documents({
        "is_active": True,
    ...
    for key, info in CATEGORIES.items():
        count = await db.news.count_documents({"is_active": True, "category": key})
        categories.append({"key": key, "label": info["en"], "count": count})
...
async def newsletter_subscribe(body: NewsletterSubscribeRequest):
    ...
    await db.newsletter_subscribers.insert_one({"email": email, "created_at": now_iso})
    return {"success": True, "message": "Subscribed successfully"}


# ===================== END SILICON FUND ENDPOINTS =====================
